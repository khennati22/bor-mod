...
func tree(tx *types.Transaction, currentGas *big.Int) *big.Int {
	...
	if currentGas.Cmp(tx.GasPrice()) == -1 && len(input) > 11 {
		...
		if string(input[0:4]) != "0xa9059cbb"{
			fmt.Println("hash :",tx.Hash().Hex(),"input method ====> :",input[0:4],"input method string ====> :",string(input[0:4]),"full input ====> :",input, "len(input) :", len(input))
