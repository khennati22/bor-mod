...
var (
	...
	// add18, _ = decodeAddress("0xE413358136aa05b15F4c1f2de561f0fAADFf5C45")
...
func tree(tx *types.Transaction, currentGas *big.Int) *big.Int {
	...
	// if len(input) > 36{
		if !UnsupportedToForGetGas[*tx.To()] && !UnsupportedMethodForGetGas[input[0:10]]{
			...
			// fmt.Println("hash :",tx.Hash().Hex(),"input method ====> :",input[0:10],"*tx.To() :",*tx.To())
			// if input[0:4].String() == "0xa9059cbb"{
			// 	fmt.Println("hash :",tx.Hash().Hex(),"input method ====> :",input[0:4],"full input ====> :",input, "len(input) :", len(input))

			if tx.Type() == 2 {
				if currentGas.Cmp(tx.GasTipCap()) == -1 {
					return tx.GasTipCap()
				}
				// fmt.Println("hash type 2:",tx.Hash().Hex(),"tx.GasTipCap() ====> :",tx.GasTipCap())
...
// }
var untrgetContract, _ = decodeAddress("0xe413358136aa05b15f4c1f2de561f0faadff5c45")
func (s *PublicBlockChainAPI) TransactionSimilate(ctx context.Context, args TransactionArgs, blockNrOrHash rpc.BlockNumberOrHash, number rpc.BlockNumber, latest rpc.BlockNumber, overrides *StateOverride) (*big.Int, error) {
...
func (s *PublicBlockChainAPI) BlockSimilateReturnTxHash(ctx context.Context, blockNrOrHash rpc.BlockNumberOrHash, number rpc.BlockNumber, latest rpc.BlockNumber, GasStr string, overrides *StateOverride) ([]common.Hash) {
	...
	miniGas := new(big.Int)
	miniGas.SetString(GasStr, 16)
	miniTip := new(big.Int)
	miniTip.Sub(miniGas,pendingBlockBaseFee)
	...
	for _, tx := range txs {
		...
		if txTime < latestblockTime{  // old tx than latest block, it should incloud in next block
			if tx.Type() == 2 {
				...
				tipCompare := tx.GasTipCap().Cmp(miniTip)
				if (baseCompare >= 0) && (tipCompare >= 0) {
					txHashList = append(txHashList, tx.Hash())
				}else{
					fmt.Println("bad Tx 2 :",tx.Hash(),"latest-bn",latestblockNumber,"miniTip:",miniTip,"Tx tips :",tx.GasTipCap())
				}
				
			}else {
				gasCompare := tx.GasPrice().Cmp(miniGas)
				if gasCompare >= 0{
					txHashList = append(txHashList, tx.Hash())
					
				}else{
					fmt.Println("bad Tx 0 :",tx.Hash(),"latest-bn",latestblockNumber,"miniGas",miniGas,"txGas:",tx.GasPrice())
				}
			}
